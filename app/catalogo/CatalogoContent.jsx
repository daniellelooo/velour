...
export default function CatalogoContent() {
  ...
  const productosFiltrados = useMemo(() => {
    ...
    let resultado = filtrarProductos({
      linea,
      tallaSeleccionada,
      precioMin,
      precioMax
    ...
    if (filtros.busqueda && filtros.busqueda.trim()) {
      ...
      resultado = resultado.filter(
        (p) =>
          ...
          (p.colores && p.colores.some((c) => c.toLowerCase().includes(termino)))
      );
    }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="mb-8">
          <p className="text-sm text-neutral-600">
            ...
            {filtros.busqueda && (
              <span className="ml-1">para &ldquo;<strong>{filtros.busqueda}</strong>&rdquo;</span>
            )}
          </p>
        </div>
        ...
        {productosFiltrados.length > 0 ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-neutral-600 mb-6">
              Intenta ajustar los filtros o la búsqueda para ver más opciones
            </p>
            <button
              onClick={() => setFiltros({ linea: '', talla: '', precioMin: 0, precioMax: 0, busqueda: '' })}
