...
export default function GraciasPage() {
  ...
  const numeroFormateado = '+57 ' + (numeroWhatsApp.replace('57', '').replace(/(\d{3})(\d{3})(\d{4})/, '$1 $2 $3'));

  const [pedido, setPedido] = useState(null);

  useEffect(() => {
    try {
      const datos = sessionStorage.getItem('velour-pedido');
      if (datos) {
        setPedido(JSON.parse(datos));
        // Limpiar para que no persista si el usuario recarga
        sessionStorage.removeItem('velour-pedido');
      }
    } catch (e) {
      console.error('Error al leer resumen del pedido:', e);
    }
  }, []);
      ...
      <div className="max-w-2xl w-full mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-4xl md:text-5xl font-light text-velour-black mb-4">
            {pedido ? `¡Gracias, ${pedido.nombre.split(' ')[0]}!` : '¡Gracias por tu pedido!'}
          </h1>
        ...
        {/* Resumen del pedido (si hay datos) */}
        {pedido && (
          <div className="bg-white p-8 shadow-sm mb-8">
            <h2 className="text-lg font-medium text-velour-black mb-5">Resumen de tu pedido</h2>
              ...
              {pedido.productos.map((item, i) => (
                <div key={i} className="flex justify-between text-sm">
                  <span className="text-neutral-700">
                    {item.nombre}
                    <span className="text-neutral-400 ml-2">Talla {item.talla} × {item.cantidad}</span>
                  </span>
                  <span className="font-medium text-velour-black">
                    {formatearPrecioConSimbolo(item.precio * item.cantidad)}
                  </span>
                </div>
              ))}
            </div>
            ...
            {/* Total */}
            <div className="flex justify-between font-medium text-base mb-5">
              <span className="text-velour-black">Total</span>
              <span className="text-velour-black">{formatearPrecioConSimbolo(pedido.total)}</span>
            </div>

            {/* Datos de envío */}
            <div className="bg-neutral-50 p-4 text-sm text-neutral-600 space-y-1">
              <p><strong className="text-neutral-800">Nombre:</strong> {pedido.nombre}</p>
              <p><strong className="text-neutral-800">Ciudad:</strong> {pedido.ciudad}</p>
              <p><strong className="text-neutral-800">Dirección:</strong> {pedido.direccion}</p>
              <p><strong className="text-neutral-800">Fecha:</strong> {pedido.fecha}</p>
            </div>
          </div>
        )}
        ...
        {/* Contacto directo */}
        <div className="bg-neutral-50 p-6 rounded mb-8 text-center">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <Link
            href="/"
            className="inline-block text-center px-8 py-3 bg-velour-black text-white hover:bg-velour-charcoal transition-colors uppercase tracking-wider text-sm font-medium"
          >
            Volver al inicio
          </Link>
          <Link
            href="/catalogo"
            className="inline-block text-center px-8 py-3 border-2 border-velour-black text-velour-black hover:bg-velour-black hover:text-white transition-colors uppercase tracking-wider text-sm font-medium"
          >
            Seguir comprando
          </Link>
        </div>
